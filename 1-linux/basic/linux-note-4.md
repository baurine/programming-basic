...
因为我们要通过 RedHat 的安装光盘来进行安装。所以要先把 CDROM 挂载到系统。(现在的图形界面已经可以自动挂载了。)
...
rpm 包的依赖性关系，yum 可以自动解决依赖性关系 (后端用的还是 rpm)。

光盘的 Server/repodata/ 目录里记录了包的依赖关系。
...
rpm 可以用来查询已安装的命令或文件是由哪个 rpm 包提供的，而 yum 都可以 (??)。
